...
export default function AdminKesraD() {
  ...
  const fetchData = async (pageNumber = 1, keywords = "") => {
    ...
    await Api.get(
      `/api/admin/beasiswa/kesra4?search=${keywords}&page=${page}&status_ketrima=${selectStatusKetrima}`,
      {
        //header
        headers: {
          //header Bearer + Token
  ...
  const handleViewSpjmt = (kesraD) => {
    const imagespjmt = kesraD?.user?.imagespjmt;
    ...
    setSpjmtUrl(fileUrl);
    setSpjmtName(`SPJMT - ${kesraD.user.name}`);
                              ...
                              kesraDs.length > 0 ? (
                                kesraDs.map((kesraD, index) => (
                                  ...
                                  >
                                    ...
                                    <td className="text-center">
                                      {(kesraD.user.status_ketrima === "1" || kesraD.user.status_ketrima === 1) ? (
                                    ...
                                    <td className="text-center">
                                      {(kesraD.user.status_ketrima === "1" || kesraD.user.status_ketrima === 1) && (
                                        checkHasSpjmtFile(kesraD.user.imagespjmt) ? (
                                          <button
                                            onClick={() => handleViewSpjmt(kesraD)}
